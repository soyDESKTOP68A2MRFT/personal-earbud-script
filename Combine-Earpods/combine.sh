...
sync_audifonos() {
  ...
  pactl load-module module-combine-sink slaves=bluez_sink.<MAC>.a2dp_sink,bluez_sink.<MAC>.a2dp_sink
  if [ $? -eq 0 ]; then
...
connect_audifonos() {
  ...
  if [ $? -eq 0 ]; then
    ...
    sleep 5
    pactl set-card-profile bluez_card.bluez_sink.<MAC> a2dp_sink
  else
